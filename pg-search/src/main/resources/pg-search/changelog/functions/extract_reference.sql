...
AS $$
  WITH paths as (select p.element, (p.path->>'value')::text[] val_path FROM search.subpaths(_type, _path, 'reference') p),
       ...
       url_parts as (SELECT regexp_split_to_array(trim(both '/' from data.d#>>val_path), '/')::text[] parts FROM data where
          data.d is not null and data.d#>>val_path is not null
          and not (data.d#>>val_path like '%?%')) -- it should be validated, but just to make sure no conditional references get to this point

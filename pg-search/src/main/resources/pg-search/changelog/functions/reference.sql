CREATE OR REPLACE FUNCTION search.reference(r store.resource, path text) RETURNS text[] AS $$
  WITH data as (SELECT unnest(search.jsonpath(r.content, path)) d),
       paths as (select (p->>'value')::text[] val FROM search.subpaths(r.type, path, 'reference') p),
